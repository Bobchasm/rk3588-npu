...
bool CpuLinear::init(int K, int N, const uint16_t* weight_kn) {
    ...
    if (K <= 0 || N <= 0 || !weight_kn) {
        std::fprintf(stderr, "[worker-pc/CpuLinear] invalid init args K=%d N=%d\n", K, N);
        return false;
    }

    K_ = K;
    N_ = N;
    weight_.assign(weight_kn, weight_kn + static_cast<size_t>(K_) * N_);
    scratch_.resize(N_);
    return true;
}
...
bool CpuLinear::forward(const uint16_t* input_f16, int M, uint16_t* output_f16) {
    if (!input_f16 || !output_f16 || K_ <= 0 || N_ <= 0 || M <= 0) {
        std::fprintf(stderr, "[worker-pc/CpuLinear] invalid forward args M=%d K=%d N=%d\n",
                     M, K_, N_);
        return false;
    }
    ...
    for (int m = 0; m < M; ++m) {
        ...
        const uint16_t* in_row = input_f16 + static_cast<size_t>(m) * K_;
        for (int k = 0; k < K_; ++k) {
            const float x = f16_to_f32(in_row[k]);
            const uint16_t* w_row = weight_.data() + static_cast<size_t>(k) * N_;
            for (int n = 0; n < N_; ++n) {
                scratch_[n] += x * f16_to_f32(w_row[n]);
            }
        }

        uint16_t* out_row = output_f16 + static_cast<size_t>(m) * N_;
        for (int n = 0; n < N_; ++n) {
